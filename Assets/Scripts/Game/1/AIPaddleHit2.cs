...
public class AIPaddleHit2 : MonoBehaviour
{
    [Header("References")]
    public Transform ball;
    public Rigidbody ballRb;
    public Transform playerCourtCenter;
    ...
    private void Start()
    {
        if (!ball || !ballRb || !playerCourtCenter)
        {
            Debug.LogError("❌ AIPaddleHit2: Reference not assigned properly!");
            enabled = false;
            return;
        }
    ...
    private void Update()
    {
        if (!ball || !ballRb || !playerCourtCenter) return;
        ...
        if (controller != null)
        {
            controller.RegisterHit("AI");
            if (controller.IsServe()) controller.ForceEndServe();
        }

        // 🎯 ramdom nearby target
        Vector3 randomizedTarget = playerCourtCenter.position;
        ...
        direction.z = Mathf.Min(direction.z, -0.5f);
        direction.y = 0.75f;
